...
Bot.prototype.connect = function() {
  ...
  }, this.options.ircOptions);

  var slackOptions = _.assign({
    partNotices: false,
    joinNotices: false,
    quitNotices: false
...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('join', function(channel, nick) {
    if (this.options.ircOptions.welcomeMessage) {
      this.ircClient.say(channel, this.options.ircOptions.welcomeMessage);
    }
    if (this.options.slackOptions.joinNotices) {
      this.sendToSlack(this.nickname, channel, nick + ' has joined the ' + channel + ' channel');
    }
  }.bind(this));

  this.ircClient.on('part', function(channel, nick, arg) {
    if (this.options.slackOptions.partNotices) {
      this.sendToSlack(this.nickname, channel, nick + ' has left the ' + channel + ' channel');
    }
  }.bind(this));

  this.ircClient.on('quit', function(nick, reason, channels, message) {
    if (this.options.slackOptions.quitNotices) {
      var index;
      for (index = 0; index < this.channels.length; index++) {
        var msg = nick + ' has quit the ' + this.channels[index] + ' channel';
        this.sendToSlack(this.nickname, this.channels[index], msg);
      }
    }
  }.bind(this));
